...
def main():
    ...
    for population in MODERN_POP:
        numer, denom, D_stat, filtered_matrix = get_D_stat(matrix, population)
        print(f"D(YRI, {population}; Altai, Vindija) = {D_stat}")

        Z_score = get_Z_score(filtered_matrix, population, numer, denom, D_stat)
        print(f"Z(YRI, {population}; Altai, Vindija) = {Z_score}")
    print()
